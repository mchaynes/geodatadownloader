...
export default function DownloadsUpdateForm(props) {
  const {
    id: idProp,
    downloads: downloadsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = downloadsRecord
      ? { ...initialValues, ...downloadsRecord, downloadscheduleID }
      : initialValues;
    setDownloadscheduleID(cleanValues.downloadscheduleID);
    setCurrentDownloadscheduleIDValue(undefined);
    setCurrentDownloadscheduleIDDisplayValue("");
    setStatus(cleanValues.status);
    setStarted_at(cleanValues.started_at);
    setFinished_at(cleanValues.finished_at);
    setMessages(cleanValues.messages ?? []);
    setCurrentMessagesValue("");
    setErrors({});
  };
  const [downloadsRecord, setDownloadsRecord] =
    React.useState(downloadsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Downloads, idProp)
        : downloadsModelProp;
      setDownloadsRecord(record);
      const downloadscheduleIDRecord = record
        ? await record.downloadscheduleID
        : undefined;
      setDownloadscheduleID(downloadscheduleIDRecord);
    };
    queryData();
  }, [idProp, downloadsModelProp]);
  React.useEffect(resetStateValues, [downloadsRecord, downloadscheduleID]);
  const [
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Downloads.copyOf(downloadsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "DownloadsUpdateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
      ...
      >
        <Autocomplete
          ...
          defaultValue={downloadscheduleID}
          onChange={(e) => {
            let { value } = e.target;
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || downloadsModelProp)}
